...
function printPageForStop($stop) {
	...
	$result = getJson("http://onebusaway.gatech.edu/api/api/where/schedule-for-stop/" . $agency . "_" . $sid . ".json?key=TEST&date=2015-04-01"); // weekday
	...
	$result = getJson("http://onebusaway.gatech.edu/api/api/where/schedule-for-stop/" . $agency . "_" . $sid . ".json?key=TEST&date=2015-04-04"); // saturday
	createSchedules($result, $stopSchedules, 'sat');
	
	// get sunday schedules
	$result = getJson("http://onebusaway.gatech.edu/api/api/where/schedule-for-stop/" . $agency . "_" . $sid . ".json?key=TEST&date=2015-04-05"); // sunday
...
function printPageHeader($stopName) {
	echo <<<EOT
	<div class='header'>
		<img class='arrow' height='80px' src='tt-logo.png'/>
		<h2>Buses departing from.. </h2>
		<h1>$stopName</h1>
	</div>
EOT;
}
...
function printPageFooter($stopId) {
	echo <<<EOT
	<div class='footer'>
		<div class='ack' style='width: 600px'>
			<p style='margin-top: 0;'>Thanks to these organizations for making this project possible:</p>
			...
			<img src='tt-logo-big.png' class='logo-big'/> 
			<p style='clear:left'> Thanks to Jeffrey Kirsch for his generous donation to this project.</p>
		</div>
		<div class='QR'>
			<img height='120px' src='qr.php?p=http://atlanta.onebusaway.org/where/standard/stop.action?id=MARTA_$stopId'/>
			<p style='clear:left'>Scan this to get live bus predictions on your mobile device</p> <!-- TODO mention OBA close to this code -->
		</div>
...
<script type='text/javascript'>
	...
	$('div.bus').each(function() { var $b = $(this); var $t = $b.find('div.title'); var w = $b.find('table.schedule').width(); $b.width(w); $t.width(w); });
