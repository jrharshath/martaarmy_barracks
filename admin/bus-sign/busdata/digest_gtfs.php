...
function generateDataForAgency($agency) {
	...
	$stopid_to_tripids = mapStopidToTripids($agency);
	...
	// export stopid_to_routeids
	$stopid_to_routeids = array();
	foreach($stopid_to_tripids as $stopid=>$tripids) {
		$routeids = array();
		foreach($tripids as $tripid) {
			addIfNotPresent($routeids, $tripid_to_routeid[$tripid]);
		}
		$stopid_to_routeids[$stopid] = $routeids;
	}
	$DATA_TO_EXPORT['stopid_to_routeids'] = $stopid_to_routeids;
	...
	$DATA_TO_EXPORT['stop_data'] = $stop_data;
...
function mapStopidToTripids($agency) {
	...
	while(($row = fgetcsv($stop_times, 1000, ",")) !== FALSE) {
		...
		addToArray($stopid_to_tripids, $stopid, $tripid);
		$num++;
	...
	if(($precompiled_file = fopen($precompiled_filename, 'w')) === FALSE) {
		...
		$precompiled = json_encode($stopid_to_tripids);
		fwrite($precompiled_file, $precompiled);
		fclose($precompiled_file);
		echo "Saved stopid_to_tripds.\n";
	}
...
function processStopData($agency) {
	...
	while(($row = fgetcsv($stops, 1000, ",")) !== FALSE) {
		if($num == 1) { 
			...
			echo "DEBUG: stop_id index = $STOPID_INDEX, stop_name index = $STOPNAME_INDEX, ".
